"""Mask-sweep test loop for the GPU testing container.

This is a harness-local adaptation of `mimose.testing.pipeline.run_testing`.
It exists here, outside `src/mimose`, because `mimose.testing` is
deliberately excluded from every exported `.mimosepkg` (see
`EXCLUDED_TOP_LEVEL_DIRS` in `src/mimose/model_export.py`) -- the
mask-sweep/Dice/HD95 scoring loop must never ship inside a package a model
owner hands over; only what's needed to reconstruct the model and call
`predict()` does. Everything this module imports from `mimose` (datasets,
checkpoints, models, enums, utils.seed) still ships inside `code/mimose.pyz`
and is resolved from there, exactly like `harness/load.py` already does --
only the loop/metrics themselves are a harness-local copy.
"""
...
# Fixed BraTS-GLI acquisition grid, mirrored from `mimose.testing.pipeline` --
# the HD95 empty-vs-nonempty fallback penalty must be the diagonal of this
# full, uncropped volume, not of whatever region happens to be loaded.
...
def run_testing_loop(
    *,
    ...
    """Reconstruct the model from an already-`resolve_package`d `.mimosepkg`
    and run the full 15-mask Dice/HD95 sweep against `data_dir` (preprocessed
    `.npz` cases, see `harness/preprocess.py`). Writes the same
    `output_path` report / `.xlsx` summary / `_per_subject.csv` shape as
    `mimose test`'s own `run_testing` used to produce when it still ran from
    inside the package.
    """
    from mimose.checkpoints import load_weights_only_checkpoint
    from mimose.datasets import IMFuseDataset
    ...
    try:
        ...
        test_set = IMFuseDataset(root=data_dir, masking_mode=None, split=split)
        ...
        with torch.no_grad():
            with Progress(
                ...
                task_id = progress.add_task("Testing masks", total=total_steps, metrics="")
                for mask in MASKS:
                    ...
                    for batch in test_loader:
                        ...
                        brats_dice_separate, brats_dice = dice_fn(output=prediction, target=target)
                        brats_hd95 = softmax_output_hd95_class4(output=prediction, target=target)
                        brats_hd95_separate = hd95_separate_fn(output=prediction, target=target)
...
def _load_test_split(*, split_file: Path, dataset_type: Any, fold: int | None = None) -> list[dict[str, Any]]:
