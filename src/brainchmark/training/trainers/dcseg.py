...
from brainchmark.losses.dcseg import (
...
class DCSegTrainer(BaseTrainer):
    def __init__(
        ...
        self.transform_kind = TransformKind(
            trainer_kwargs.get("transform_kind", TransformKind.DCSEG)
        )
    ...
    def _train_step(self, batch: dict[str, Any], epoch: int) -> dict[str, float]:
        ...
        with self._autocast_context():
            ...
            prm_total = metrics["prmcross"] + metrics["prmdice"]
            sep_total = metrics["sepcross"] + metrics["sepdice"]
            fuse_total = metrics["fusecross"] + metrics["fusedice"]
            loss = prm_total
            if self.use_reg_loss:
                loss = loss + sep_total
            if epoch >= self.region_fusion_start_epoch:
                loss = loss + fuse_total
            ...
            anatomical_contrastive_loss = self.anatomy_contrastive_loss(contents)
            modality_contrastive_loss = self.modality_contrastive_loss(styles)
