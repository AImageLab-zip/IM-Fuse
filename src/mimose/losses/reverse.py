...
class ReverseLoss:
    """Loss wrapper matching legacy Reverse's fuse + sep + prm + RFIM consistency terms."""
    ...
    def training_loss(
        ...
        # "reverse" self-consistency loss: inverting the forward mapping
        ...
        for global_tokens, inverse_out in zip(global_tokens_used, inverse_outputs):
            reverse_mse = reverse_mse + F.mse_loss(inverse_out, global_tokens, reduction="mean")

        total = (
            (self.sep_weight * sep["total"])
            + (self.prm_weight * prm["total"])
            + (self.forward_weight * forward_mse)
            + (self.reverse_weight * reverse_mse)
        )
        if include_fuse:
