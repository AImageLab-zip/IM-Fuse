...
class Decoder_fuse(nn.Module):
    def __init__(self, num_cls: int = 4) -> None:
        ...
        self.RFM1 = fusion_prenorm(in_channel=basic_dims, num_cls=num_cls)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkv = (
            self.qkv(x)
            .reshape(batch_size, tokens, 3, self.num_heads, channels // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
        for layer_index in range(self.depth):
            x = x + pos
            x = self.cross_attention_list[layer_index](x)
            x = self.cross_ffn_list[layer_index](x)
        return x
