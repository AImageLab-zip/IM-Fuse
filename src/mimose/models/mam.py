...
# Per-stage channel counts, matching legacy MaM's nnU-Net "3d_fullres" plan at
...
class MaM(AbstractModel):
    """Implementation of MaM introduced in [1].
    ...
    def forward(
        ...
        if x.size(1) != num_modals:
            raise RuntimeError(f"MaM expects {num_modals} input modalities, got {x.size(1)}")
        if mask.ndim != 2 or mask.size(1) != num_modals:
            raise RuntimeError(f"MaM expects mask shape [B, {num_modals}], got {tuple(mask.shape)}")
