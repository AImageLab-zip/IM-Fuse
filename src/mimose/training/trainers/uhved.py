...
class UHVEDTrainer(BaseTrainer):
    def __init__(
        self,
        input_dir: Path,
        output_dir: Path,
    ...
    def train_epoch(self, epoch: int) -> dict[str, float]:
        ...
        totals = {"loss": 0.0, "cross": 0.0, "dice": 0.0, "kld": 0.0, "recon": 0.0}
    ...
    def _train_step(self, batch: dict[str, Any]) -> dict[str, float]:
        ...
        with self._autocast_context():
            target = self._seg_to_one_hot(seg)
            outputs, post_param = self.model(images, mask)
            metrics = self._loss_impl().training_loss(outputs, post_param, target, mask)
            loss = metrics["loss"]
