...
class MSTKDTrainer(BaseTrainer):
    def __init__(
        self,
        input_dir: Path,
        output_dir: Path,
    ...
    def _train_step(self, batch: dict[str, Any], epoch: int) -> dict[str, float]:
        ...
        with self._autocast_context():
            ...
            metrics = self._loss_impl().training_loss(outputs, target, epoch)
            loss = metrics["loss"]

        self._backward_step(loss)
        return {
            "loss": float(loss.item()),
            "loss_dc": float(metrics["loss_dc"].item()),
            "loss_miss_dc": float(metrics["loss_miss_dc"].item()),
            "consistency": float(metrics["consistency"].item()),
            "content": float(metrics["content"].item()),
            "style": float(metrics["style"].item()),
            "gsm": float(metrics["gsm"].item()),
            "unetr": float(metrics["unetr"].item()),
            "evd": float(metrics["evd"].item()),
            "slkd": float(metrics["slkd"].item()),
        }
