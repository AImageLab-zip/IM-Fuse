...
class SRMNetTrainer(BaseTrainer):
    def __init__(
        self,
        input_dir: Path,
        output_dir: Path,
    ...
    def _train_step(self, batch: dict[str, Any]) -> dict[str, float]:
        ...
        return {
            ...
            "pred_cross": float(metrics["pred_cross"].item()),
            "pred_dice": float(metrics["pred_dice"].item()),
            "preds_cross": float(metrics["preds_cross"].item()),
            "preds_dice": float(metrics["preds_dice"].item()),
