...
import scipy.misc
from utils import Parser,criterions_imfuse
...
def test_softmax(
    ...
    for i, data in enumerate(test_loader): 
        ...
        if isinstance(names,list):
            names = [Path(name).name.split('.')[0] for name in names]
        else:
            names = Path(names).name.split('.')[0] 
        yo = target
        ...
        for h in h_idx_list:
            for w in w_idx_list:
                for z in z_idx_list:
                    ...
                    pred = output[0, :, :240, :240, :155].cpu().numpy().argmax(0)

        
        # segmentation loss 
        if compute_loss:
            seg_cross_loss = criterions_imfuse.softmax_weighted_loss(pred, yo, num_cls=num_cls)
            seg_dice_loss = criterions_imfuse.dice_loss(pred, yo, num_cls=num_cls)
