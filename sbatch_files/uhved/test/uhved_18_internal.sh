#!/bin/bash
#SBATCH --job-name=UHVED18_TEST_INTERNAL
...
#SBATCH -e /homes/ocarpentiero/slurm/outerr/uhved_18_test_internal.err
#SBATCH -o /homes/ocarpentiero/slurm/outerr/uhved_18_test_internal.out
#SBATCH --gres=gpu:1
...
for trained_fold in 1 3 5; do
    mimose test \
        --config uhved_18.yaml \
        --run-suffix "fold${trained_fold}" \
        --dataset-type internal \
        --split-file "$SPLIT_FILE" \
        --fold 1 \
        --data-dir "$DATA_DIR" \
        --output-path "/work/phd_mimose/results/uhved_18/results_internal.txt"
done
