#!/bin/bash
#SBATCH --job-name=SFUSION18_TEST_INTERNAL
...
#SBATCH -e /homes/ocarpentiero/slurm/outerr/sfusion_18_test_internal.err
#SBATCH -o /homes/ocarpentiero/slurm/outerr/sfusion_18_test_internal.out
#SBATCH --gres=gpu:1
...
# Tests the brats18-trained sfusion checkpoints (folds 1/3/5) against
...
for trained_fold in 1 3 5; do
    mimose test \
        --config sfusion_18.yaml \
        --run-suffix "fold${trained_fold}" \
        --dataset-type internal \
        --split-file "$SPLIT_FILE" \
        --fold 1 \
        --data-dir "$DATA_DIR" \
        --output-path "/work/phd_mimose/results/sfusion_18/results_internal.txt"
done
