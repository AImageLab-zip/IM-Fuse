#!/bin/bash
#SBATCH --job-name=SIMPLEUNET23_TEST_INTERNAL
...
#SBATCH -e /homes/ocarpentiero/slurm/outerr/simpleunet_23_test_internal.err
#SBATCH -o /homes/ocarpentiero/slurm/outerr/simpleunet_23_test_internal.out
#SBATCH --gres=gpu:1
...
# Tests the brats23-trained simpleunet checkpoints (folds 1/3/5) against
...
for trained_fold in 1 3 5; do
    mimose test \
        --config simpleunet_23.yaml \
        --run-suffix "fold${trained_fold}" \
        --dataset-type internal \
        --split-file "$SPLIT_FILE" \
        --fold 1 \
        --data-dir "$DATA_DIR" \
        --output-path "/work/phd_mimose/results/simpleunet_23/results_internal.txt"
done
