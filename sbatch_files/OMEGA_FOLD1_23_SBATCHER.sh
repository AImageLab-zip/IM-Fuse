#!/usr/bin/env bash
# Submit the FOLD 1 combined (train+test) script for EVERY model's BraTS23 run under
# sbatch_files/*/all/ (i.e. *_23_1.sh only). Each script is submitted 4x with a combined
...
shopt -s nullglob
scripts=("${script_dir}"/*/all/*_23_1.sh)
if [ "${#scripts[@]}" -eq 0 ]; then
    echo "No fold-1 BraTS23 (*_23_1.sh) all scripts found under ${script_dir}/*/all/" >&2
    exit 1
fi
