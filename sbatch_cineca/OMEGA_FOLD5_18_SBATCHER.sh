#!/usr/bin/env bash
# Submit the FOLD 5 combined (train+test) script for EVERY model's BraTS18 run under
# sbatch_files/*/all/ (i.e. *_18_5.sh only). Each script is submitted 4x with singleton
...
shopt -s nullglob
scripts=("${script_dir}"/*/all/*_18_5.sh)
if [ "${#scripts[@]}" -eq 0 ]; then
    echo "No fold-5 BraTS18 (*_18_5.sh) all scripts found under ${script_dir}/*/all/" >&2
    exit 1
fi
