#!/usr/bin/env bash
# Submit the FOLD 3 combined (train+test) script for EVERY model's BraTS23 run under
# sbatch_files/*/all/ (i.e. *_23_3.sh only). Each script is submitted 4x with a combined
...
shopt -s nullglob
scripts=("${script_dir}"/*/all/*_23_3.sh)
if [ "${#scripts[@]}" -eq 0 ]; then
    echo "No fold-3 BraTS23 (*_23_3.sh) all scripts found under ${script_dir}/*/all/" >&2
    exit 1
fi
