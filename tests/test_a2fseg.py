...
def test_a2fseg_training_forward_returns_fuse_and_sep_predictions() -> None:
    model = A2FSeg(num_cls=4).eval()
...
def test_a2fseg_fusion_softmax_ignores_missing_modalities() -> None:
    torch.manual_seed(0)
    model = A2FSeg(num_cls=4).eval()
    images = torch.randn(1, 4, 16, 16, 16)
    full_mask = torch.ones(1, 4, dtype=torch.bool)
    partial_mask = torch.tensor([[True, False, True, False]])

    with torch.no_grad():
        full_output = model(images, full_mask)
        partial_output = model(images, partial_mask)

    # Different present-modality subsets must route through different
    # softmax-gated weights, so the fused prediction should differ.
    assert not torch.allclose(full_output, partial_output)
...
def test_a2fseg_resolves_from_model_config_builder() -> None:
    model_config = build_model_config("a2fseg", {"num_cls": 4})

    assert model_config.model_class is A2FSeg
...
@pytest.mark.parametrize("config_name", ["a2fseg_18.yaml", "a2fseg_23.yaml"])
def test_packaged_a2fseg_configs_use_imfuse_training_contract(config_name: str) -> None:
...
def test_packaged_2023_configs_include_a2fseg_for_flops_all() -> None:
    config_names = {path.name for path in resolve_2023_config_paths()}

    assert "a2fseg_23.yaml" in config_names
