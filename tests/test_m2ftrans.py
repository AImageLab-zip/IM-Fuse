...
from mimose.models.m2ftrans import M2FTrans, input_patch_size
...
def test_m2ftrans_inference_forward_returns_segmentation_shape() -> None:
    torch.manual_seed(0)
    model = M2FTrans(num_cls=4).eval()
...
def test_m2ftrans_training_forward_returns_fuse_sep_and_prm_predictions() -> None:
    torch.manual_seed(0)
    model = M2FTrans(num_cls=4).eval()
...
def test_m2ftrans_handles_missing_modalities() -> None:
    torch.manual_seed(0)
    model = M2FTrans(num_cls=4).eval()
    images = torch.randn(1, 4, input_patch_size, input_patch_size, input_patch_size)
...
def test_m2ftrans_remaps_dataset_modalities_to_legacy_order() -> None:
    images = torch.arange(4, dtype=torch.float32).reshape(1, 4, 1, 1, 1)
    mask = torch.tensor([[False, True, False, True]])

    remapped_images, remapped_mask = M2FTrans._remap_input_order(images, mask)

    assert remapped_images.flatten().tolist() == [2.0, 0.0, 1.0, 3.0]
    assert remapped_mask.tolist() == [[False, False, True, True]]


def test_m2ftrans_window_starts_use_50_percent_overlap() -> None:
    assert M2FTrans._window_starts(input_patch_size) == [0]
    assert M2FTrans._window_starts(input_patch_size + 40) == [0, 40]

    with pytest.raises(RuntimeError, match="at least 80"):
        M2FTrans._window_starts(input_patch_size - 1)
...
def test_m2ftrans_rejects_invalid_modality_or_mask_shape(
    images: torch.Tensor,
    mask: torch.Tensor,
) -> None:
    model = M2FTrans(num_cls=4).eval()

    with pytest.raises(RuntimeError, match="M2FTrans expects"):
        model(images, mask)


def test_m2ftrans_resolves_from_model_config_builder() -> None:
    model_config = build_model_config("m2ftrans", {"num_cls": 4})

    assert model_config.model_class is M2FTrans
    assert model_config.kwargs == {"num_cls": 4}


def test_m2ftrans_transform_manager_resolves_from_kind() -> None:
...
@pytest.mark.parametrize("config_name", ["m2ftrans_18.yaml", "m2ftrans_23.yaml"])
def test_packaged_m2ftrans_configs_use_imfuse_training_contract(config_name: str) -> None:
    config = load_yaml_config(Path(config_name))

    assert config["trainer"] == "imfuse"
    assert config["model"] == "m2ftrans"
