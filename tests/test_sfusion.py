...
from mimose.models.sfusion import SFusion, input_patch_size
...
def test_sfusion_inference_forward_returns_segmentation_shape() -> None:
    torch.manual_seed(0)
    model = SFusion(num_cls=4).eval()
...
def test_sfusion_training_forward_returns_outputs_dict() -> None:
    torch.manual_seed(0)
    model = SFusion(num_cls=4).eval()
    ...
    assert outputs["seg_pred"].shape == expected_shape
    assert outputs["seg_logit"].shape == expected_shape
    for modality in ("Flair", "T1c", "T1", "T2"):
        ...
        assert outputs[f"mu_{modality}"].shape == (1, 64, 1, 1, 1)
        assert outputs[f"sigma_{modality}"].shape == (1, 64, 1, 1, 1)
        assert outputs["images"][modality].shape == (1, 1, input_patch_size, input_patch_size, input_patch_size)


def test_sfusion_handles_missing_modalities() -> None:
    torch.manual_seed(0)
    model = SFusion(num_cls=4).eval()
    images = torch.randn(1, 4, input_patch_size, input_patch_size, input_patch_size)
...
def test_sfusion_remaps_dataset_modalities_to_legacy_order() -> None:
    images = torch.arange(4, dtype=torch.float32).reshape(1, 4, 1, 1, 1)
    mask = torch.tensor([[False, True, False, True]])

    remapped_images, remapped_mask = SFusion._remap_input_order(images, mask)

    assert remapped_images.flatten().tolist() == [2.0, 0.0, 1.0, 3.0]
    assert remapped_mask.tolist() == [[False, False, True, True]]


def test_sfusion_window_starts_use_50_percent_overlap() -> None:
    assert SFusion._window_starts(input_patch_size) == [0]
    assert SFusion._window_starts(input_patch_size + 64) == [0, 64]

    with pytest.raises(RuntimeError, match=f"at least {input_patch_size}"):
        SFusion._window_starts(input_patch_size - 1)
...
def test_sfusion_rejects_invalid_modality_or_mask_shape(
    images: torch.Tensor,
    mask: torch.Tensor,
) -> None:
    model = SFusion(num_cls=4).eval()

    with pytest.raises(RuntimeError, match="SFusion expects"):
        model(images, mask)


def test_sfusion_resolves_from_model_config_builder() -> None:
    model_config = build_model_config("sfusion", {"num_cls": 4})

    assert model_config.model_class is SFusion
    assert model_config.kwargs == {"num_cls": 4}


def test_sfusion_reuses_robustseg_loss() -> None:
...
def test_sfusion_transform_manager_resolves_from_kind() -> None:
...
def test_sfusion_training_loss_end_to_end() -> None:
    torch.manual_seed(4)
    model = SFusion(num_cls=4)
    model.is_training = True
    images = torch.randn(1, 4, input_patch_size, input_patch_size, input_patch_size)
    mask = torch.ones(1, 4, dtype=torch.bool)
...
@pytest.mark.parametrize("config_name", ["sfusion_18.yaml", "sfusion_23.yaml", "sfusion_25.yaml"])
def test_packaged_sfusion_configs_use_robustseg_training_contract(config_name: str) -> None:
    config = load_yaml_config(Path(config_name))

    assert config["trainer"] == "robustseg"
    assert config["model"] == "sfusion"
