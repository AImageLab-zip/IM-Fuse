...
from mimose.models.m3fecon import M3FeCon, input_patch_size
...
def test_m3fecon_inference_forward_returns_segmentation_shape() -> None:
    torch.manual_seed(0)
    model = M3FeCon(num_cls=4).eval()
...
def test_m3fecon_training_forward_returns_bottleneck_and_ground_truth() -> None:
    torch.manual_seed(0)
    model = M3FeCon(num_cls=4).eval()
...
def test_m3fecon_handles_missing_modalities_and_heterogeneous_batch_masks() -> None:
    torch.manual_seed(0)
    model = M3FeCon(num_cls=4).eval()
...
def test_m3fecon_rejects_invalid_modality_or_mask_shape(
    images: torch.Tensor,
    mask: torch.Tensor,
) -> None:
    model = M3FeCon(num_cls=4).eval()

    with pytest.raises(RuntimeError, match="M3FeCon expects"):
        model(images, mask)


def test_m3fecon_window_starts_use_50_percent_overlap() -> None:
    assert M3FeCon._window_starts(input_patch_size) == [0]
    assert M3FeCon._window_starts(input_patch_size + 64) == [0, 64]

    with pytest.raises(RuntimeError, match=f"at least {input_patch_size}"):
        M3FeCon._window_starts(input_patch_size - 1)


def test_m3fecon_resolves_from_model_config_builder() -> None:
    model_config = build_model_config("m3fecon", {"num_cls": 4})

    assert model_config.model_class is M3FeCon
    assert model_config.kwargs == {"num_cls": 4}


def test_m3fecon_loss_resolves_from_loss_config_builder() -> None:
    loss_config = build_loss_config("m3fecon", {"num_classes": 4})

    assert loss_config.loss_class is M3FeConLoss
    assert loss_config.kwargs == {"num_classes": 4}


def test_m3fecon_transform_manager_resolves_from_kind() -> None:
...
def test_m3fecon_reconstruction_loss_only_covers_missing_modalities() -> None:
    torch.manual_seed(4)
    model = M3FeCon(num_cls=4)
    model.is_training = True
    images = torch.randn(1, 4, input_patch_size, input_patch_size, input_patch_size)
    ...
    loss_fn = M3FeConLoss(num_classes=4)
...
def test_m3fecon_training_loss_end_to_end_with_missing_modality() -> None:
    torch.manual_seed(4)
    model = M3FeCon(num_cls=4)
    model.is_training = True
    images = torch.randn(1, 4, input_patch_size, input_patch_size, input_patch_size)
...
@pytest.mark.parametrize("config_name", ["m3fecon_18.yaml", "m3fecon_23.yaml", "m3fecon_25.yaml"])
def test_packaged_m3fecon_configs_use_m3fecon_training_contract(config_name: str) -> None:
    config = load_yaml_config(Path(config_name))

    assert config["trainer"] == "m3fecon"
    assert config["model"] == "m3fecon"
    assert config["loss"] == "m3fecon"
