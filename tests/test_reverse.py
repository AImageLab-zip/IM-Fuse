...
from mimose.models.reverse import Reverse, input_patch_size
...
def test_reverse_inference_forward_returns_segmentation_shape() -> None:
    torch.manual_seed(0)
    model = Reverse(num_cls=4).eval()
...
def test_reverse_training_forward_returns_fuse_sep_prm_and_rfim_outputs() -> None:
    torch.manual_seed(0)
    model = Reverse(num_cls=4).eval()
...
def test_reverse_handles_missing_modalities() -> None:
    torch.manual_seed(0)
    model = Reverse(num_cls=4).eval()
    images = torch.randn(1, 4, input_patch_size, input_patch_size, input_patch_size)
...
def test_reverse_remaps_dataset_modalities_to_legacy_order() -> None:
    images = torch.arange(4, dtype=torch.float32).reshape(1, 4, 1, 1, 1)
    mask = torch.tensor([[False, True, False, True]])

    remapped_images, remapped_mask = Reverse._remap_input_order(images, mask)

    assert remapped_images.flatten().tolist() == [2.0, 0.0, 1.0, 3.0]
    assert remapped_mask.tolist() == [[False, False, True, True]]


def test_reverse_window_starts_use_50_percent_overlap() -> None:
    assert Reverse._window_starts(input_patch_size) == [0]
    assert Reverse._window_starts(input_patch_size + 64) == [0, 64]

    with pytest.raises(RuntimeError, match=f"at least {input_patch_size}"):
        Reverse._window_starts(input_patch_size - 1)
...
def test_reverse_rejects_invalid_modality_or_mask_shape(
    images: torch.Tensor,
    mask: torch.Tensor,
) -> None:
    model = Reverse(num_cls=4).eval()

    with pytest.raises(RuntimeError, match="Reverse expects"):
        model(images, mask)


def test_reverse_resolves_from_model_config_builder() -> None:
    model_config = build_model_config("reverse", {"num_cls": 4})

    assert model_config.model_class is Reverse
    assert model_config.kwargs == {"num_cls": 4}


def test_reverse_loss_resolves_from_loss_config_builder() -> None:
    loss_config = build_loss_config("reverse", {"num_classes": 4})

    assert loss_config.loss_class is ReverseLoss
    assert loss_config.kwargs == {"num_classes": 4}


def test_reverse_transform_manager_resolves_from_kind() -> None:
...
def test_reverse_training_loss_end_to_end() -> None:
    torch.manual_seed(4)
    model = Reverse(num_cls=4)
    model.is_training = True
    images = torch.randn(1, 4, input_patch_size, input_patch_size, input_patch_size)
    ...
    outputs = model(images, mask)
    loss_fn = ReverseLoss(num_classes=4)
...
def test_reverse_training_loss_zeroes_fuse_term_when_excluded() -> None:
    torch.manual_seed(4)
    model = Reverse(num_cls=4)
    model.is_training = True
    images = torch.randn(1, 4, input_patch_size, input_patch_size, input_patch_size)
...
@pytest.mark.parametrize("config_name", ["reverse_18.yaml", "reverse_23.yaml", "reverse_25.yaml"])
def test_packaged_reverse_configs_use_reverse_training_contract(config_name: str) -> None:
    config = load_yaml_config(Path(config_name))

    assert config["trainer"] == "reverse"
    assert config["model"] == "reverse"
    assert config["loss"] == "reverse"
