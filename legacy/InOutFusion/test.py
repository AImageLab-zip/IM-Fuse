...
from utils.util import re_crop
...
model = RsInOut_U_Hemis3D(in_channels=1, out_channels=4,
                           levels=4, feature_maps=8, method='TF', phase='test').to(DEVICE)
model.eval()
...
with torch.no_grad():
    for i, mask in tqdm(enumerate(masks),desc='Evaluating all the masks'):
        ...
        for element in tqdm(test_loader,total=len(test_loader),desc=f'Testing: {mask_names[i]}'):
            target = element[8].type(torch.float32).to(DEVICE).detach()
            m_d = element[6].to(DEVICE).detach()
            crop_size = element[7]
            image = []
            for k in range(4):
                image.append(element[k].type(torch.float32).to(DEVICE).unsqueeze(0).detach())

            output = model(image,m_d)

            output = F.softmax(output,dim=1)
            output = torch.argmax(output, dim=1)
            output_restored = re_crop(output.squeeze(0).squeeze(0),crop_size)
            output_restored = output_restored.unsqueeze(0)
            
            _ , brats_dice = softmax_output_dice_class4(output=output_restored,target=target)
