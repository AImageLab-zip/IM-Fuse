...
def main():
    ...
    #valid_cache = Cache(data_file_path=Path(args.datapath), splitfile=Path(__file__).parent / 'datalist' / 'val15splits.csv')
    #valid_set = BraTS_TrainCache(cache=valid_cache, testing=True)
    try:
        ...
        #print(f"Training samples: {len(train_set)} | Validation samples: {len(valid_set)}")
        ...
        for epoch in tqdm(range(start_epoch, num_epochs),desc='Training the network',position=0):
            model.train()
            running_loss = 0.0
            ...
            for batch_idx, data in tqdm(enumerate(train_loader),total=len(train_loader),position=1):
                #gc.collect()
                ...
                total_seg_loss = torch.zeros(1, device='cuda') # Should be total_seg_loss = torch.tensor(0.0).cuda() - homarone00
            ...
            # -------------------------
            # 6. Validation per epoch
            # -------------------------
            '''with torch.no_grad():
                dice_mean, hd95_mean = validate_softmax(
                    train_valid_loader=valid_loader,
                    model=model,
                    epoch_id=epoch,
            ...
            # Save every 25 epochs or at the final epoch
