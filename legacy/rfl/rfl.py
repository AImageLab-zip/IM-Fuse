...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.rfl_flair = RFIM(dim=transformer_basic_dims)
        self.rfl_t1 = RFIM(dim=transformer_basic_dims)
        self.rfl_t1ce = RFIM(dim=transformer_basic_dims)
        self.rfl_t2 = RFIM(dim=transformer_basic_dims)
        self.rflBlock_half_flair=RFIM_half(dim=transformer_basic_dims)
        self.rflBlock_half_t1=RFIM_half(dim=transformer_basic_dims)
        self.rflBlock_half_t1ce=RFIM_half(dim=transformer_basic_dims)
        self.rflBlock_half_t2=RFIM_half(dim=transformer_basic_dims)

    def forward(self, x, mask):
        # if t1c is present, use t1ce as global modality; otherwise, use the first available modality as global modality
        if mask[0][1]:
            global_modality = 1
        else :
            for i in range(4):
        ...
        if self.is_training:
            flair_rfl_forward_out, flair_inverse_out = self.rfl_flair(intra_token_x5[global_modality])
            t1_rfl_forward_out, t1_inverse_out = self.rfl_t1(intra_token_x5[global_modality])
            t1ce_rfl_forward_out, t1ce_inverse_out = self.rfl_t1ce(intra_token_x5[global_modality])
            t2_rfl_forward_out, t2_inverse_out = self.rfl_t2(intra_token_x5[global_modality])
            rfl_forward_out=[flair_rfl_forward_out, t1_rfl_forward_out, t1ce_rfl_forward_out, t2_rfl_forward_out]
            ...
            if mask[0][0] is False:
                flair_intra_x5 = flair_rfl_forward_out.view(x.size(0), patch_size, patch_size, patch_size, transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
            if mask[0][1] is False:
                t1_intra_x5 = t1_rfl_forward_out.view(x.size(0), patch_size, patch_size, patch_size, transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
            if mask[0][2] is False:
                t1ce_intra_x5 = t1ce_rfl_forward_out.view(x.size(0), patch_size, patch_size, patch_size, transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
            if mask[0][3] is False:
                t2_intra_x5 = t2_rfl_forward_out.view(x.size(0), patch_size, patch_size, patch_size, transformer_basic_dims).permute(0, 4, 1, 2, 3).contiguous()
        ...
        if not self.is_training:
            ...
            if mask[0][0] is False:
                ...
                flair_inverse_out=self.rflBlock_half_flair(flair_intra_token_x5)
            if mask[0][1] is False:
                t1_intra_token_x5 = intra_token_x5[global_modality]
                t1_inverse_out = self.rflBlock_half_t1(t1_intra_token_x5)
            if mask[0][2] is False:
                t1ce_intra_token_x5 = intra_token_x5[global_modality]
                t1ce_inverse_out = self.rflBlock_half_t1ce(t1ce_intra_token_x5)
            if mask[0][3] is False:
                t2_intra_token_x5 = intra_token_x5[global_modality]
                t2_inverse_out = self.rflBlock_half_t2(t2_intra_token_x5)
        ...
        if self.is_training:
            return (fuse_pred, (flair_pred, t1ce_pred, t1_pred, t2_pred), preds, intra_token_x5[global_modality], rfl_forward_out,
