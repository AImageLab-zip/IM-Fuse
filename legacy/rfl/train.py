...
def main():
    ...
    wandb_name_and_id = f'{args.dataname}_rfl_epoch{args.num_epochs}_iter{args.iter_per_epoch}_jobid{slurm_job_id}'
    if not args.debug:
        wandb.init(
    ...
    model = rfl.Model(num_cls=num_cls)
    print(model)
    ...
    if args.dataname in ['BRATS2023']:
        ...
        raise NotImplementedError('dataset is not supported yet')
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        rfl_mse_loss_epoch = 0.0
        forward_mse_loss_epoch = 0.0
        loss_epoch = 0.0
        ...
        for i in range(iter_per_epoch):
            ...
            fuse_pred, sep_preds, prm_preds, global_modality, rfl_forward_out, rfl_out, intra_token_x5 = model(x, mask)
            ...
            forward_mse_loss = 0
            rfl_mse_loss = 0
            # rfl output similar to the global modality representation
            for j in range(4):
                rfl_mse_loss = rfl_mse_loss + mse_loss(global_modality, rfl_out[j])
                # tokens generated for modality j must be similar to the corresponding tokens
            for j in range(4):
                forward_mse_loss = forward_mse_loss + mse_loss(intra_token_x5[j], rfl_forward_out[j])

            rfl_mse_loss_epoch += rfl_mse_loss
            forward_mse_loss_epoch += forward_mse_loss
            ...
            if epoch < args.region_fusion_start_epoch:
                loss = fuse_loss * 0.0 + sep_loss + prm_loss + forward_mse_loss + rfl_mse_loss
            else:
                loss = fuse_loss + sep_loss + prm_loss + forward_mse_loss + rfl_mse_loss
            ...
            writer.add_scalar('rfl_mse_loss', rfl_mse_loss.item(), global_step=step)
            writer.add_scalar('forward_mse_loss', forward_mse_loss.item(), global_step=step)
            ...
            msg += 'rfl_mse:{:.4f}, forward_mse:{:.4f}'.format(rfl_mse_loss.item(), forward_mse_loss.item())
            logging.info(msg)
        ...
        if not args.debug:
            wandb.log({
                ...
                "train/rfl_mse": rfl_mse_loss_epoch.cpu().detach().item() / iter_per_epoch,
                "train/forward_mse": forward_mse_loss_epoch.cpu().detach().item() / iter_per_epoch,
