...
if __name__ == '__main__':
    ...
    resume = '/work/grana_neuro/missing_modalities/IMS2Trans/output18/output_6/best.pth'
    num_cls = 4
    dataname = 'BRATS2018'
    ...
    model = ims2trans.Model(num_cls=num_cls, use_checkpoint=False)
    model = torch.nn.DataParallel(model).cuda()

    checkpoint = torch.load(resume)
    model.load_state_dict(checkpoint['state_dict'])
    best_epoch = checkpoint['epoch'] + 1
    output_path = f"eval/output_{dataname}_{best_epoch}.txt"
