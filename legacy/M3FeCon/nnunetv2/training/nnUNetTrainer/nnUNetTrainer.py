...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.val_epochs = [20, 50, 70, 100, 120, 150, 200, 250, 300, 350, 370, 390, 400, 420, 450, 460, 470, 480, 485, 490, 495, 499, 500]
        self.num_val_iterations_per_epoch = 125
        self.num_epochs = 500
        ...
        self.csv_path = "/work/grana_neuro/missing_modalities/mmFormer/BRATS2023_Training_npy/datalist/val15splits.csv"

        # Build a dictionary from case -> [bool, bool, bool, bool] for the test cases
        self.maskdict = {}
        with open(self.csv_path, "r") as f:
            reader = csv.DictReader(f)  # expects "case,mask" headers
            for row in reader:
                case = row["case"]
                mask_str = row["mask"]

                # Convert the string "[False, False, False, True]" to a Python list
                # ast.literal_eval safely evaluates strings containing Python literals
                mask_list = ast.literal_eval(mask_str)  # -> [False, False, False, True]
                self.maskdict[case] = mask_list
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            args = [input_tensor.to(self.device), mask]
            ...
            params_list = []
            measure_list = []
            flops, macs, _ = calculate_flops(model=self.network,
                                             args=args,
            ...
            flops_list.append(float(flops.split(" ")[0]))
            measure_list.append(flops.split(" ")[1])
    ...
    def on_train_start(self):
        ...
        wandb_name_and_id = f'BraTS23_M3FeCon_epoch{self.num_epochs}_jobid{slurm_job_id}'
        wandb.init(
            ...
            resume="allow",
            config={
                "architecture": "Missing as Masking",
                "learning_rate": self.initial_lr,
                "batch_size": self.batch_size,
                "iter_per_epoch": self.num_iterations_per_epoch,
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        wandb.log({
            ...
            "val/val_ET_Dice": global_dc_per_class[2],
            "val/val_WT_Dice": global_dc_per_class[0],
            "val/val_TC_Dice": global_dc_per_class[1],
            "val/val_Dice": mean_fg_dice,
            "val/seg_loss": loss_here,
        })

        if self.best_dice is None or mean_fg_dice > self.best_dice:
            self.best_dice = mean_fg_dice
            self.print_to_log_file(f"Yayy! New best average Dice: {np.round(self.best_dice, decimals=4)}")
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('Dice scores (WT, TC, ET)', [np.round(i, decimals=4) for i in
    ...
    def perform_actual_validation(self, save_probabilities: bool = False, mask: str = (1, 0, 1, 1)):
        self.set_deep_supervision_enabled(False)
        self.network.eval()
        mask = [int(i) for i in mask[0].split(',')]
        print('mask:', mask)
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            validation_output_folder = join(self.output_folder, 'validation' + '_' + str(mask) )
            #print('validation_output_folder:', validation_output_folder)
            ...
            for i, k in enumerate(dataset_val.keys()):
                ...
                data = np.copy(data)
                print('data shape in validation:', data.shape)
                for i, mask_index in enumerate(mask):
                    if not mask_index:
                            data[i, :, :, :] = 0 
                ...
                prediction, skip = predictor.predict_sliding_window_return_logits(data, mask)
                prediction = prediction.cpu()
                #print('out_file:', output_filename_truncated)
                '''
                /data_hdd/users/zengzhilin/nnUNetV2_trained_models/Dataset520_BraTS2020/nnUNetTrainerMissingBaseline__nnUNetPlans__3d_fullres_missing/fold_0/validation_[0, 1, 0, 1]/BraTS20_Training_032
                '''
                '''
                skip.shape: [1,320,4,4,4]
                '''
                
                #save skip as npy file as output_filename_truncated
                np.save(output_filename_truncated+'.npy', skip.cpu().numpy())
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            train_outputs = []
            print('epoch', self.current_epoch, len(self.dataloader_train), len(self.dataloader_val))
            for batch_id in range(len(self.dataloader_train)):
                train_outputs.append(self.train_step(next(self.dataloader_train)))
            self.on_train_epoch_end(train_outputs)

            with torch.no_grad():
                if self.current_epoch in self.val_epochs:
                    self.on_validation_epoch_start()
                    val_outputs = []
                    for batch_id in range(len(self.dataloader_val)):
