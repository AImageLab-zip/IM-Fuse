...
class nnUNetPredictor(object):
    ...
    def predict_from_files(self,
        ...
        return self.predict_from_data_iterator(mask, data_iterator, save_probabilities, num_processes_segmentation_export)
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                prediction = self.predict_logits_from_preprocessed_data(data, mask).cpu()
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor, mask) -> torch.Tensor:
        """
        ...
        with torch.no_grad():
            ...
            for params in self.list_of_parameters:
                ...
                if prediction is None:
                    prediction = self.predict_sliding_window_return_logits(data, mask)[0].to('cpu')
                else:
                    prediction += self.predict_sliding_window_return_logits(data, mask)[0].to('cpu')
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor, mask) -> torch.Tensor:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None
        prediction, skip,  reconGT = self.network(x, mask)
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        try:
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                prediction, skip = self._internal_maybe_mirror_and_predict(workon, mask)
            ...
            raise e
        return predicted_logits, skip

    def predict_sliding_window_return_logits(self, input_image: torch.Tensor, mask) \
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if self.perform_everything_on_device and self.device != 'cpu':
                    ...
                    try:
                        predicted_logits,skip = self._internal_predict_sliding_window_return_logits(data, mask, slicers,
                                                                                               self.perform_everything_on_device)
                    except RuntimeError:
                        print(
                            'Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                        empty_cache(self.device)
                        predicted_logits = self._internal_predict_sliding_window_return_logits(data, mask, slicers, False)
                else:
                    predicted_logits,skip = self._internal_predict_sliding_window_return_logits(data, mask, slicers,
                                                                                           self.perform_everything_on_device)
...
def predict_entry_point_modelfolder():
    ...
    parser.add_argument('-f', nargs='+', type=str, required=False, default=(0, 1, 2, 3, 4),
                        help='Specify the folds of the trained model that should be used for prediction. '
                             'Default: (0, 1, 2, 3, 4)')
    parser.add_argument('-mask', nargs='+', type=str, required=False, default=(1, 1, 1, 1),
                        help='Specify the folds of the trained model that should be used for prediction. '
                             'Default: (0, 1, 2, 3, 4)')
...
def predict_entry_point():
    ...
    parser.add_argument('-f', nargs='+', type=str, required=False, default=(0, 1, 2, 3, 4),
                        help='Specify the folds of the trained model that should be used for prediction. '
                             'Default: (0, 1, 2, 3, 4)')
    parser.add_argument('-mask', nargs='+', type=str, required=False, default=(1, 1, 1, 1),
                        help='Specify the availale modality combinations '
                             'Default: (1, 1, 1, 1)')
    parser.add_argument('-step_size', type=float, required=False, default=0.5,
                        help='Step size for sliding window prediction. The larger it is the faster but less accurate '
                             'the prediction. Default: 0.5. Cannot be larger than 1. We recommend the default.')
    ...
    assert args.part_id < args.num_parts, 'Do you even read the documentation? See nnUNetv2_predict.'
    ...
    raw_mask = args.mask[0]  # something like "0,1,0,0"
    mask_list = [int(x) for x in raw_mask.split(',')]
    bool_mask = [x == 1 for x in mask_list]
    predictor.predict_from_files(args.i, args.o, save_probabilities=args.save_probabilities,
                                 overwrite=not args.continue_prediction,
                                 num_processes_preprocessing=args.npp,
                                 num_processes_segmentation_export=args.nps,
