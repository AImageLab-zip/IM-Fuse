...
def run_stage(cfg: StageConfig):
    ...
    if prev_model is not None and modes_to_load:
        ...
        for m in modes_to_load:
            rb = _load_replay(cfg.base_dir, m, device='cpu')
            if rb is None:
                continue
    ...
    for epoch in range(start_epoch, cfg.max_epoch):
        ...
        for batch in tqdm(train_loader, total=int(len(train_loader)/cfg.batch_size), desc=f'Training epoch {epoch} of stage {cfg.img_mode}'):
            imgs_orig = batch['image']
            imgs = imgs_orig.to(device=device, dtype=torch.float32,memory_format=torch.channels_last)
            masks = batch['mask'].to(device=device, dtype=torch.float32,memory_format=torch.channels_last)
            slice_ids = batch['idx']
        ...
        if cfg.wandb_project_name is not None:
            wandb.log(
            {
                ...
                "train/dice_micro": train_dice_micro,
                "train/dice_avg": train_dice_avg,
                "train/dice_WT": train_WT_avg,
                "train/dice_TC": train_TC_avg,
                "train/dice_ET": train_ET_avg,

                # Validation
                "val/loss": val_loss,
                "val/dice_micro": val_dice_micro,
                "val/dice_avg": val_dice_avg,
                "val/dice_WT": WT_avg,
                "val/dice_TC": TC_avg,
                "val/dice_ET": ET_avg,
    ...
    if len(best_loss_by_name) > 0:
        ...
        for s in range(0, len(sel_names), CHUNK):
            ...
            imgs_np = [_to_chw(np.load(img_dir / f"{n}.npz")['data']) for n in part]
            masks_np = [_to_chw(np.load(mask_dir / f"{n}.npz")['data']) for n in part]
