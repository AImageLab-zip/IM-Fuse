...
def calculate_mse_loss(original_tokens, reconstructed_tokens, presences):
    mse_losses = []
    original_tokens = original_tokens.transpose(0, 1)  # shape: (4, 1, 128, 4, 4, 4)
    reconstructed_tokens = reconstructed_tokens.transpose(0, 1)
...
class nnUNetTrainerMissingReconBaseline(nnUNetTrainer):
    def _build_loss(self):
        if self.label_manager.has_regions:
            print('has regions:', self.label_manager.has_regions)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            mseloss_list = calculate_mse_loss(recon_Gt, recon, selected_mask)
            if len(mseloss_list) == 0:# DEBUGGA QUESTO!!!!!!!!
                mseloss = torch.tensor(0.0, device=recon.device, dtype=recon.dtype)
            else:
                mseloss = torch.stack(mseloss_list).mean()
            combined_loss = l + mseloss
        if self.grad_scaler is not None:
            ...
            torch.nn.utils.clip_grad_norm_(self.network.parameters(), 12)
            self.optimizer.step()
        return {'seg_loss': l.detach().cpu().numpy(), 'mse_loss': mseloss.detach().cpu().numpy(), 'loss': combined_loss.detach().cpu().numpy()}
    
    def validation_step(self, batch: dict) -> dict:
        data = batch['data']
        target = batch['target']
        case_name = batch['keys'][0]

        #selected_mask = random.choice(masks_total)
        selected_mask = self.maskdict[case_name]
        # for i, mask in enumerate(selected_mask):
        #     if not mask:
        ...
        if self.label_manager.has_regions:
            predicted_segmentation_onehot = (torch.sigmoid(output) > 0.5).long() #(1, 3, 128, 128, 128)
        else:
            # no need for softmax
