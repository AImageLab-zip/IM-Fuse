...
class TF_3D(nn.Module):
    def __init__(self, embedding_dim=1024, volumn_size=8, nhead=4, num_layers=8, method='TF'):
        ...
        if method=='Token':
            self.fusion_token = nn.Parameter(torch.zeros((1, self.patch_dim ** 3, self.d_model)))

    def forward(self, all_content):
        n_modality = len(all_content)
        token_content = self.project(all_content) #(B, 512, 8), (B, 512, 16), (B, 512, 32), (512, 64)
        check_values(token_content, name='token_content')
        ...
        check_values(out, name='out')
        atten_map = self.reproject(out, n_modality, self.method) #(N, B, 8, 128, 128, 128), (N, B, 16, 64, 64, 64), (N, B, 16, 64, 64, 64), (N, B, 32, 32, 32, 32), (N, B, 64, 16, 16, 16)
        check_values(atten_map, name='atten_map')
    ...
    def project(self, all_content):
        ...
        for i in range(n_modality):
            content = self.avgpool(all_content[i]) #(B, 8, 8, 8, 8)
            content = content.permute(0, 2, 3, 4, 1).contiguous()
            content2 = content.view(content.size(0), -1, self.embedding_dim) #(B, 512, 8)
            if i == 0:
                token_content_in = content2
            else:
                token_content_in = torch.cat([token_content_in, content2], dim=1)
        check_values(token_content_in, name='token_content_in')
    ...
    def reproject(self, atten_map, n_modality, method):
        ...
        #a_m = torch.nan_to_num(a_m, nan=0.0, posinf=1.0, neginf=-1.0)  # Replace NaN/Inf
        check_values(a_m, name='a_m')
        return a_m
    ...
    def atten(self, all_content, atten_map, n_modality):
        ...
        for i in range(n_modality):
            ...
            if output == None:
                ...
                output += all_content[i] * a_m
        check_values(output, name='output_atten')
        return output
...
class DUpsampling3D(nn.Module):
    ...
    def forward(self, x):
        ...
        check_values(x, name='x_DUpsampling3D')
        return x
...
class PositionalEncoding(nn.Module):
    ...
    def _get_sinusoid_encoding_table(self, n_position, d_hid):
        def get_position_angle_vec(position):
            ...
            return [position / (np.power(10000, 2 * (hid_j // 2) / d_hid) + epsilon) for hid_j in range(d_hid)]

        sinusoid_table = np.array([get_position_angle_vec(pos_i) for pos_i in range(n_position)])
        sinusoid_table[:, 0::2] = np.sin(sinusoid_table[:, 0::2])
        sinusoid_table[:, 1::2] = np.cos(sinusoid_table[:, 1::2])
        output_tensor = torch.FloatTensor(sinusoid_table).unsqueeze(0)
        check_values(output_tensor, name='sinusoid_PositionalEncoding')
        return output_tensor
    

    def forward(self, x):
        output = x + self.pos_table[:, :x.size(1)].clone().detach().to(x.device)#to(torch.device('cuda'))
        check_values(output, name='output_PositionalEncoding')
        return output
...
class style_encoder(nn.Module):
    ...
    def forward(self, inputs):
        output = self.unit(inputs) #[B,128,9,9,0]
        output = torch.mean(output, dim=(2, 3, 4), keepdim=True) #(B, 128, 1, 1, 1)
...
class mask_decoder(nn.Module):
    ...
    def forward(self, inp):
        ...
        seg_pred = self.seg_pred(seg) #(B, num_cls=4, 128, 128, 128)
