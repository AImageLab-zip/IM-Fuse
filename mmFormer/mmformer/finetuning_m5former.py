...
from data.datasets_nii import Brats_loadall_nii_loc, Brats_loadall_test_nii_loc, Brats_loadall_val_nii_loc
from data.data_utils import init_fn
from utils import Parser,criterions
from utils.parser import setup 
from utils.lr_scheduler import LR_Scheduler, record_loss, MultiEpochsDataLoader 
from predict import AverageMeter, test_softmax_limage, mask_modal
...
val_check = [1, 5, 10, 15, 20, 30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 160, 170, 180, 190, 200] 
...
def main():
    ...
    wandb_name_and_id = f'BraTS23_m5Former_finetuning_epoch{args.num_epochs}_iter{args.iter_per_epoch}_jobid{slurm_job_id}'
    if not args.debug:
        wandb.init(
    ...
    checkpoint = torch.load('/work/grana_neuro/missing_modalities/mmFormer/output/best.pth')
    ...
    limage = ck['state_dict']['module.limage'].detach().cuda() #(1, 4, 155, 240, 240)
    ...
    train_set = Brats_loadall_nii_loc(transforms=args.train_transforms, root=args.datapath, num_cls=num_cls, train_file=train_file)
    test_set = Brats_loadall_test_nii_loc(transforms=args.test_transforms, root=args.datapath, test_file=test_file)
    val_set = Brats_loadall_val_nii_loc(transforms=args.test_transforms, root=args.datapath, num_cls=num_cls, val_file=val_file)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i in range(iter_per_epoch):
            ...
            x, target, mask, x_min, x_max, y_min, y_max, z_min, z_max, name = data #x=(B, M=4, 128, 128, 128), target = (B, C, 128, 128, 128), mask = (B, 4)
            x = x.cuda(non_blocking=True)
            limage_cropped = limage[:, :, z_min:z_max, y_min:y_max, x_min:x_max].permute(0, 1, 4, 3, 2).cuda()
            target = target.cuda(non_blocking=True)
            mask = mask.cuda(non_blocking=True)

            fuse_pred, sep_preds, prm_preds = model(x, limage_cropped, mask)
        ...
        if (epoch+1) in val_check or args.debug:
            print('validate ...')
            with torch.no_grad():
                dice_score, seg_loss = test_softmax_limage(
                    val_loader,
                    model,
