...
def test_softmax_limage(
    ...
    for i, data in enumerate(test_loader): 
        x, target, mask, yo, x_min, x_max, y_min, y_max, z_min, z_max, names = data
        target = target.cuda()
        x = x.cuda()
        yo = yo.cuda()
            ...
            for h in h_idx_list:
                for w in w_idx_list:
                    for z in z_idx_list:
                        ...
                        limage_input = limage_cropped[:, :, h:h+patch_size, w:w+patch_size, z:z+patch_size]
                        pred_part = model(x_input, limage_input, mask)
