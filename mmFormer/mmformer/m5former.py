...
class ViewModal(nn.Module):
    def __init__(self):
        super(ViewModal, self).__init__()
...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.proj_flair = nn.Conv3d(1, 1, kernel_size=1, stride=1, padding=0)
        self.proj_t1 = nn.Conv3d(1, 1, kernel_size=1, stride=1, padding=0)
        self.proj_t1c = nn.Conv3d(1, 1, kernel_size=1, stride=1, padding=0)
        self.proj_t2 = nn.Conv3d(1, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, limage_cropped, mask):
        #flair_proj = self.proj_flair(limage_cropped[:, 0])
        #t1c_proj = self.proj_t1c(limage_cropped[:, 1])
        #t1_proj = self.proj_t1(limage_cropped[:, 2])
        #t2_proj = self.proj_t2(limage_cropped[:, 3])
        #limage_cropped = torch.stack((flair_proj, t1c_proj, t1_proj, t2_proj), dim=1)

        x_input = self.masker(x, mask) + self.masker(limage_cropped, ~mask) 
        
        #extract feature from different layers
        flair_x1, flair_x2, flair_x3, flair_x4, flair_x5 = self.flair_encoder(x_input[:, 0:1, :, :, :])
        t1ce_x1, t1ce_x2, t1ce_x3, t1ce_x4, t1ce_x5 = self.t1ce_encoder(x_input[:, 1:2, :, :, :])
        t1_x1, t1_x2, t1_x3, t1_x4, t1_x5 = self.t1_encoder(x_input[:, 2:3, :, :, :])
        t2_x1, t2_x2, t2_x3, t2_x4, t2_x5 = self.t2_encoder(x_input[:, 3:4, :, :, :])
        ...
        x1 = self.viewer(torch.stack((flair_x1, t1ce_x1, t1_x1, t2_x1), dim=1)) #Bx4xCxHWZ = (B, 32, 128, 128)
        x2 = self.viewer(torch.stack((flair_x2, t1ce_x2, t1_x2, t2_x2), dim=1))
        x3 = self.viewer(torch.stack((flair_x3, t1ce_x3, t1_x3, t2_x3), dim=1))
        x4 = self.viewer(torch.stack((flair_x4, t1ce_x4, t1_x4, t2_x4), dim=1))
        x5_intra = self.viewer(torch.stack((flair_intra_x5, t1ce_intra_x5, t1_intra_x5, t2_intra_x5), dim=1)) #(1, 2048, 8, 8, 8)
        ...
        multimodal_inter_x5 = self.multimodal_decode_conv(multimodal_inter_token_x5.view(multimodal_inter_token_x5.size(0), patch_size, patch_size, patch_size, transformer_basic_dims*num_modals).permute(0, 4, 1, 2, 3).contiguous()) #(B, 2048, 8, 8, 8) -> (B, 512, 8, 8, 8)
