...
def _load_and_compress(args):
    idx, image_dict = args
    flairNII = nib.load(image_dict["flair"]) #TODO
    t1NII = nib.load(image_dict["t1"])
    t1ceNII = nib.load(image_dict["t1ce"])
    t2NII = nib.load(image_dict["t2"])
    labelNII = nib.load(image_dict["label"])
    ...
    data = np.concatenate(
    [flair[None], t1[None], t1ce[None], t2[None], label],
    axis=0
    ).astype(np.float32)

    comp = lz4.frame.compress(data.tobytes())
    return idx, (comp, data.shape, data.dtype.str)
...
def id2trainId(label):
    ...
    return results_map
class Cache:
    def __init__(self, datalist, length, num_workers=None):

        tasks = [(i, p) for i, p in enumerate(datalist)]
        ...
        self.cache = {} 
        self.list_of_names = []
        for idx, (comp, shape, dtype_str) in tqdm(results,desc='Building the Cache'):
        ...
        self.cache = list(self.cache.values()) * length
    def __getitem__(self, idx): 
        sh_name, shape, dtype,size = self.cache[idx]
...
class BraTSDataSet(data.Dataset):
    def __init__(self,datapath:Path, list_path, max_iters, crop_size=(128, 160, 200), scale=True, mirror=True, ignore_label=255):
        ...
        for item in self.img_ids:
            ...
            flair_file = str(filepath) + '-t2f.nii.gz'
            t1_file = str(filepath) + '-t1n.nii.gz'
            t1ce_file = str(filepath) + '-t1c.nii.gz'
            t2_file = str(filepath) + '-t2w.nii.gz'
            label_file = str(filepath) + '-seg.nii.gz'
        ...
        self.cache = Cache(self.files,int(np.ceil(float(max_iters) / len(self.img_ids))))
...
class BraTSValDataSet(data.Dataset):
    def __init__(self, datapath:Path, list_path):
        self.list_path = list_path
        self.img_ids = [i_id.strip() for i_id in open(self.list_path)]
        self.img_ids = [[datapath / img] for img in self.img_ids]  
        self.files = []
...
def my_collate(batch):
    ...
    print(patch_size)
    tr_transforms = get_train_transform(patch_size=patch_size)
