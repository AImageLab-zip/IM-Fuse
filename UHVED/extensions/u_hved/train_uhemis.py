...
from u_hemis_net_torch import U_HeMIS
...
val_check = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 90, 100, 110, 125, 150, 175, 200, 225, 250, 260, 270, 275, 280, 285, 290, 295, 300] 
...
def main():
    ...
    wandb_name_and_id = f'BraTS23_UHEMIS_epoch{args.num_epochs}_lr{args.lr}_jobid{slurm_job_id}'
    if not args.debug:
        wandb.init(
    ...
    ##########setting models
    num_cls = 4
    model = U_HeMIS(num_classes=num_cls)
    print (model)
    model = torch.nn.DataParallel(model).cuda()

    ########## Setting learning scheduler and optimizer
    lr_scheduler = LR_Scheduler(args.lr)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i in range(iter_per_epoch):
            ...
            images = dict() #(flair, t1ce, t1, t2)
            images['Flair'] = x[:,0:1,:,:,:]
            images['T1c'] = x[:,1:2,:,:,:]
            images['T1'] = x[:,2:3,:,:,:]
            images['T2'] = x[:,3:4,:,:,:]

            net_seg = model(images, mask)
            
            ### Loss computation
            loss_cross = softmax_loss(net_seg, target, num_cls=num_cls) 
            loss_dice = dice_loss(net_seg, target, num_cls=num_cls)
            loss = loss_cross + loss_dice
        ...
        if epoch+1 in val_check or args.debug:
            ...
            logging.info('Testing epoch = {}, WT = {:.2}, TC = {:.2}, ET = {:.2}, ET_pp = {:.2}, loss = {:.2}'.format(epoch, test_WT.item(), test_TC.item(), test_ET.item(), test_ETpp.item(), seg_loss.cpu().item()))
