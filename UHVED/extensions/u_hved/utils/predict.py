...
import scipy.misc
from utils.criterions import cross_loss, dice_loss, softmax_dice_loss, softmax_loss, softmax_weighted_loss
...
def test_softmax(
    ...
    for i, data in tqdm(enumerate(test_loader)): 
        ...
        for h in h_idx_list:
            for w in w_idx_list:
                for z in z_idx_list:
                    ...
                    pred_part = model(images, mask, is_inference=True)
        ...
        ##### segmentation loss 
        if compute_loss:
            seg_cross_loss = softmax_loss(pred, yo, num_cls=num_cls) 
            seg_cross_loss = softmax_weighted_loss(pred, yo, num_cls=num_cls) 
            seg_dice_loss = dice_loss(pred, yo, num_cls=num_cls)
    ...
    for i, data in tqdm(enumerate(test_loader)): 
        ...
        for h in h_idx_list:
            for w in w_idx_list:
                for z in z_idx_list:
                    x_input = x[:, :, h:h+patch_size, w:w+patch_size, z:z+patch_size]
                    pred_part = model(x_input, mask, is_training=False)
                    pred[:, :, h:h+patch_size, w:w+patch_size, z:z+patch_size] += pred_part
        pred = pred / weight
        b = time.time()
        pred = pred[:, :, :H, :W, :T]
