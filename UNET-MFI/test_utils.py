...
def softmax_output_dice_class4(output, target):
    eps = 1e-8
    assert len(output.shape) == 4, f'Wrong shape for network output: {output.shape} instead of (155, 240, 240)'
    assert len(target.shape) == 4, f'Wrong shape for segmentation target: {target.shape} instead of (155, 240, 240)'
    output = output.cpu().unsqueeze(1)
    ...
    return None, dice_evaluate.cpu().numpy()
